...
int main(int argc, char * argv[]) {
    ...
    int size_needed = MultiByteToWideChar(CP_ACP, 0, argv[2], -1, NULL, 0);
    std::vector<wchar_t> buffer(size_needed);
    MultiByteToWideChar(CP_ACP, 0, argv[2], -1, buffer.data(), size_needed);
    std::wstring fullPortName = prefix + buffer.data();
    LPCWSTR portName = fullPortName.c_str();
  HANDLE h = CreateFile(portName/*L"\\\\.\\COM7" */ , GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
  if (!h) {
